...
<div class="view_room bg-white p-4 rounded">
    <div class="box ">
        {{-- Edit Modal --}}
        <div class="modal fade" id="editroom" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle"
            aria-hidden="true">
                ...
                <div class="modal-content">
                            ...
                            <div class="row">
                                <div class="col px-5">
                                    ...
                                    <div class="row mt-2">
                                        ...
                                        <input type="text" class="form-control input" name="hide" id="img"
                                            disabled>
                                    </div>
                                </div>
                                <div class="col">
                                    ...
                                    <div class="row mr-3 pb-1">
                                        ...
                                        <div class="dropdown" id="dropdown">
                                            <input class="form-control dropdown-toggle" type="text"
                                                ...
                                                aria-expanded="false" name="r_facility">
                                            <div class="dropdown-menu" id="dropdown-menu"
                                                aria-labelledby="dropdownMenuButton">
                                                @if ($facilities)
                                                    @foreach ($facilities as $facility)
                                                        <li class="dropdown-item"><span class="check">✔️
                                                            </span>{{ $facility->name }}</li>
                                                    @endforeach
                                                @else
                                                    <li class="dropdown-item">No Available</li>
                                                @endif
                                    ...
                                    <div class="row mr-3 pb-1">
                                        ...
                                        <div class="dropdown" id="dropdown1">
                                            <input class="form-control dropdown-toggle" type="text"
                                                ...
                                                aria-expanded="false" name="r_feature">
                                            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                                                @if ($features)
                                                    @foreach ($features as $feature)
                                                        <li class="dropdown-item"><span class="check">✔️
                                                            </span>{{ $feature->name }}</li>
                                                    @endforeach
                                                @else
                                                    <li class="dropdown-item">No Available</li>
                                                @endif
                                    ...
                                    <div class="row mr-3 pb-1">
                                        <div class="col ml-0 px-0 mr-2">
                                            ...
                                            <input type="number" name="r_adult"
                                                id="r_adult"class="input form-control">
                                        </div>
                                        <div class="col mx-0 px-0">
                                            <label for="" class="form-label">Children</label>
                                            <input type="number" name="r_children"
                                                id="r_children"class="form-control">
                                        </div>
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal"
                            id="closeeditr">Close</button>
                        {{-- <button type="button" class="btn btn-primary" id="save_edited_room">Change</button> --}}
        ...
        <div class="header d-flex flex-md-row justify-content-between">
            <h4 class="mt-4">Room List</h4>
            {{-- <div class="form-control w-25 mr-4">
            Room Type
            <select name="" class="form-control mr-4">
                @foreach ($categories as $category)
                    <option value="{{$category->Name}}">{{$category->Name}}</option>
                @endforeach
            </select>
                ...
                @if ($rooms)
                    ...
                    @foreach ($rooms as $room)
                        <tr class="row{{ $room->id }}">
                            <td>{{ $no++ }}</td>
                            <td><img style="height:4em;width:auto;border-radius:4px"
                                    src="{{ asset('RoomImg/' . $room->image) }}" alt=""></td>
                            ...
                            @endif --}}
                            <td><button class ="update-room-btn" data-toggle="modal" data-target="#editroom"
                                    value="{{ Crypt::encryptstring($room->id) }}"><i class='fa fa-edit mr-1'
                                        style='color:#efefeb'></i></button></button><button class ="delete-room-btn"
                                    value="{{ Crypt::encryptstring($room->id) }}">
    ...
    $(document).ready(function() {
        ...
        $('#dropdown .dropdown-menu').on('click', '.selected', function() {
            ...
            if (currentValue.includes(', ' + text)) {
                ...
                let updatedValue = currentValue.replace(text + ', ', "").replace(text + ',', "")
                    .replace(", ,", ", ").trim();
                $('#dropdownMenuButton').val(updatedValue);
